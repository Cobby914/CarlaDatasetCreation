...
Run standalone (after RadarCameraSetup* and traffic are up):
    python TestRadarLabeling.py
...
from RadarLabelingTestReport import (
    DetectionRecord,
...
def format_status_line(snap: dict) -> str:
    ...
    return (
        f"{RADAR_TEST_LOG_PREFIX} scored={snap['total_detections']:,} | "
...
def resolve_output_dir(script_dir: Path, explicit: str = "") -> Path:
    """
    New folder per run: radar_labeling_test_YYYYMMDD_HHMMSS under script dir
    ...
    parent = Path(base).resolve() if base else script_dir
